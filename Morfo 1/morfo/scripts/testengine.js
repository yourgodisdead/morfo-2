...
var HTMLFilaTablaInteriorPregunta = 
		"<tr>" + 
          "<td width='10'><input type='#tipo control#' name='#nombre control#' id='#id control#' value=''></td>" +
...
var HTMLPopup = 
		...
		"<link href='file:///C|/estilos/biologicas.css' rel='stylesheet' type='text/css'>"+
		"</head>"+
...
function TerminarTest(ABoton)
{
	if(ABoton)
	{
		var accion = (this.Tipo == "e") ? "Evaluate" : "Chequiar";
		var objeto = "test";
...
{
	var titulo = (this.Tipo == "c") ? "Chequear" : "Your score";
	var html = "";
	for(var i = 0; i < this.Preguntas.length; i++)
	{
...
{
	var titulo = (this.Test.Tipo == "c") ? "Chequear" : "Your score";
	var html =  this.HTMLRespuesta();
	if(String(ACalif) != "undefined")
	{
		html += "<br><span class='" + EstiloCalificacion + "'>Calificacion: " + ACalif + "</span>";
	}
	html = HTMLPopup.replace(/#contenido#/, html);
	html = html.replace("#titulo#", titulo);
	var v = new TVentana();
